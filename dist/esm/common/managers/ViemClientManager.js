...
import { ContractFunctionExecutionError, HttpRequestError, InvalidInputRpcError, MethodNotFoundRpcError, RpcRequestError, TransactionNotFoundError, UnknownNodeError, UnknownRpcError, createPublicClient, createWalletClient, fallback } from "viem";
import { privateKeyToAccount } from "viem/accounts";
import { globalConfig } from "../../constants";
import { Logger, createCustomHttpTransport, getEnvVar } from "../utils";
import { ManagerBase } from "./ManagerBase";
export var ViemClientManager = /*#__PURE__*/ function(ManagerBase) {
    "use strict";
    ...
    function ViemClientManager(rpcManager) {
        ...
        _this.logger = Logger.getInstance().getLogger("ViemClientManager");
        return _this;
    }
    _create_class(ViemClientManager, [
        ...
        },
        {
            ...
            value: function createTransport(chain) {
                ...
                return fallback(rpcUrls.map(function(url) {
                    return createCustomHttpTransport(url);
                }), _object_spread_props(_object_spread({}, globalConfig.VIEM.FALLBACK_TRANSPORT_OPTIONS), {
                    shouldThrow: function(error) {
                        if (_instanceof(error, HttpRequestError) || _instanceof(error, RpcRequestError) || _instanceof(error, TransactionNotFoundError) || _instanceof(error, UnknownRpcError) || _instanceof(error, UnknownNodeError) || _instanceof(error, InvalidInputRpcError) || _instanceof(error, MethodNotFoundRpcError)) {
                            return false;
                        } else if (_instanceof(error, ContractFunctionExecutionError)) {
        ...
        },
        {
            ...
            value: function initializeClients(chain) {
                var privateKey = getEnvVar("OPERATOR_PRIVATE_KEY");
                var account = privateKeyToAccount("0x".concat(privateKey));
